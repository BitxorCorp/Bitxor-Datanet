...
import { bitxor, monitor } from '@src/config';
...
export class NodeMonitor {
	...
	private getNodeList = async (): Promise<any> => {
		...
		logger.info(`[getNodeList] Initial node list: ${bitxor.NODES.join(', ')}`);
		for (const nodeUrl of bitxor.NODES) {
	...
	private async fetchAndSetNetworkInfo(): Promise<void> {
		...
		logger.info(`Network identifier not found in ${bitxor.NODES}, using default ${this.networkIdentifier}`);
	}
